...
# --- 1. 数据读取与预处理 ---
intersection_data3 <- read.table("D:/0 depth sequencing/20260409-5NRR 胶回收 cut/merged/cut/ISH1/20260527/20nt_in1_M.txt", header=T, sep="\t")
...
df_base <- intersection_data3 %>%
  ...
  arrange(desc(enrichment_factor_nonoffset)) %>%
  slice_head(n = 50) %>%
...
# 保存图像
ggsave("D:/0 depth sequencing/20260409-5NRR 胶回收 cut/merged/cut/NSH/20260527/Sequence_Frequency_Logo50_Seamless.pdf", final_plot, width = 18, height = 14)